...
// for the per-kind Dart manifests and the spec's golden gallery — Dart
...
for (const entry of readdirSync(ROOT, { withFileTypes: true })) {
  ...
  // A kind is a directory of cases, which is what makes this structural rather
  // than a list of directory names to ignore: the toolchain's build output
  // comes and goes, and a list of what to skip would need editing every time
  // something new appeared beside it.
  if (!existsSync(join(ROOT, entry.name, 'cases'))) continue;
...
for (const kind of KINDS) {
    ...
    if (kind.images) {
      for (const c of cases) {
        ...
        if (kind.scoped) {
          ...
          if (!/\bregion: StageRegion\./.test(source) && !/\/\/ whole-screen: \S/.test(source)) {
            fail(
  ...
  const manifestName = `${kind.id}_manifest.GENERATED.dart`;
  compare(
...
const runner = readFileSync(join(ROOT, 'runner_test.dart'), 'utf8');
for (const kind of KINDS) {
  const name = `${kind.id}Cases`;
  if (!runner.includes(name)) fail(`kinds: runner_test.dart never executes \`${name}\``);
}
...
function renderManifest(kind, cases) {
  ...
  const imports = cases.map((c) => `import 'cases/${c.file}';`).join('\n');
...
final ${name.toLowerCase()}Cases = <RequirementCase>${entries};
`;
}
